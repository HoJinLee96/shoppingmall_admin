...
import static net.chamman.shoppingmall_admin.exception.HttpStatusCode.INTERNAL_SERVER_ERROR;
import static net.chamman.shoppingmall_admin.exception.HttpStatusCode.VALIDATION_FAILED;
...
@Slf4j
@Order(1)
@RestControllerAdvice(basePackages = {"net.chamman.shoppingmall_admin.domain","net.chamman.shoppingmall_admin.infra","net.chamman.shoppingmall_admin.global"})
...
//	public ResponseEntity<ApiResponseDto<Map<String, String>>> handleCriticalException(UserDuplicationException ex) {
...
//	}
	...
	public ResponseEntity<ApiResponseDto<Void>> handleCriticalException(CriticalException ex) {
		HttpStatusCode httpStatusCode = ex.getHttpStatusCode();
		log.error("* {} 발생.", ex.getClass().getSimpleName(), ex);
	...
	public ResponseEntity<ApiResponseDto<Void>> handleMethodArgumentNotValidException(
		...
		return ResponseEntity.status(400).body(apiResponseFactory.errorWithMessageKey(VALIDATION_FAILED, messageKey));
	}
		...
		if (messageKey != null && !messageKey.isEmpty()) {
			try {
				...
				return ResponseEntity.status(400)
						.body(apiResponseFactory.errorWithMessage(VALIDATION_FAILED, "입력 값에 문제가 있습니다."));
			}
		}
		return ResponseEntity.status(400).body(apiResponseFactory.errorWithMessage(VALIDATION_FAILED, "입력 값에 문제가 있습니다."));
	}
