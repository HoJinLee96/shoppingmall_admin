...
public class LogMaskingUtil {
	...
	public static String maskId(long id, MaskLevel level) {
	...
	public static String maskUsername(String username, MaskLevel level) {
		if (!StringUtils.hasText(username))
			return "***";
		return switch (level) {
		case STRONG -> username.substring(0, Math.min(2, username.length())) + "****";
		case MEDIUM -> username.substring(0, Math.min(5, username.length())) + "****";
		case NONE -> username;
		};
	}
